...
\tvntxt{The \sout{A}} current dipole moment ${\bf p}$ can be calculated from an axial current $I$ inside a neuron and the distance vector ${\bf d}$ traveled by the axial current: ${\bf p} = I{\bf d}$, analogous to a charge dipole moment. \tvnnote{Bruker vi ikke også den litt enklere formelen med transmembrane strømmer $p=\sum I r$?}
...
In order to save time and storage \tvnnote{trenger ikke motivasjon her syntes jeg}, we can compute a single current dipole moment for each neuron. One way of doing this is to sum up the multiple current dipole moments, in other words, to make a position-weighted sum of all the axial currents for each time point in the neuron simulation:
