...
The New York Head model is a realistic head model based on high-resolution, anatomical MRI-data from 152 adult heads \citep{HUANG2015}. The link between current dipoles in the brain and the resulting EEG signals was established taking advantage of the reciprocity theorem, stating that the position of the electrode and the dipolar source can be switched without affecting the measured potential. This means, that it is sufficient to virtually inject current at the locations of the EEG electrodes and use the finite element method \citep{LOGG2012} to compute the resulting potential anywhere in the brain. The resulting  is fully described by the \txttt{lead field} which can be downloaded from \url{parralab.org/nyhead/}. %Due to the reciprocity theorem {sitere RushandDriscoll1969}, which in short states that switching the location of recording electrode will not affect the measured potential,  This lead field matrix was computed by inserting current into all the various electrode locations, using the finite element method , 
