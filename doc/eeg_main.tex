...
\begin{figure}[H]
    ...
    100 μm (black dots) is positioned in the center of the population.
...
The four-sphere head model can be a convenient head model for some generic EEG studies, but for many applications more realistic head models will be needed \citep{DALE1999, VORWERK2014}. 
The construction of such head models typically require very expensive equpiment, such as magnetic resonance imaging (MRI) to map the electrical conductivity of the entire head, at resolutions of $\sim$0.5~mm$^3$. Afterwards, numerical techniques such as the Finite Element Method (FEM) \citep{LOGG2012} can be used to calculate the signal at the EEG electrodes for arbitrary complex head geometries and arbitrary arrangements of current dipoles in the brain, but at a high computational cost.

The reciprocity principle of Helmholtz states, in short, that switching the location of a current source and a recording electrode will not affect the measured potential \citep{Malmivuo1995, Ziegler2014, HUANG2016, Dmochowski2017}. This implies that it suffices to use FEM to calculate the lead field in the brain from a virtual current dipole at each of the EEG electrodes, and this will be sufficient to infer the potential at the electrodes, given an arbitrary arangement of current dipoles in the brain. 
Luckily, several such pre-solved complex head models are freely available, such as the New York Head that we will use here (\cite{HUANG2016}; \url{www.parralab.org/nyhead/}). \tvnnote{Cite others? Freesurfer? Sim4Life? Don Hagler?}
