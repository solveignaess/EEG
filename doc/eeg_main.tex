...
\snnote{Beklager fryktelig mange forslag til endringer (igjen). Din versjon ligger nederst, kommentert ut, så da har du alt der, hvis du savner noe. Synes innholdet er helt supert! Har prøvd å fjerne repeterende ord og gjøre en del setninger litt kortere. Jeg synes opplistinga av hovedutfordringer var veldig fin! Har gjort om litt på strukturen her, for å linke avsnitt 3 og 4 tydeligere til hovedutfordringene i avsnitt 2.}

\sntxt{\sout{Electroencephalography (EEG) is the measurement of electrical potentials on the surface of the skull,
and such measurements have been an essential part of neuroscience for nearly a century }}\citep{SILVA2013}\sntxt{\sout{. Today, such measurements are still among the most important non-invasive methods (without surgical interventions) to study human cognitive functions, as well as to diagnose and study brain diseases }}\citep{COHEN2017, Pesaran2018}.

\sntxt{
Electroencephalogram (EEG) is one of the most important non-invasive methods for studying human cognitive function and diagnosing brain diseases \citep{COHEN2017, Pesaran2018}.} \sout{Despite EEG's long history and important role in neuroscience} Yet, we \sntxt{\sout{still}} know surprisingly little about the neural origin of \sntxt{these electric potentials \sout{the signal}} \citep{COHEN2017}: \sntxt{On the one hand,} we have a relatively good understanding of the \sntxt{\sout{biophysical origin of the EEG signal:it} biophysics of EEGs, knowing that scalp potentials} mainly originate from large numbers of synaptic input\sntxt{\sout{s}} to cortical pyramidal cell populations
\citep{NUNEZ2006, SILVA2013, Pesaran2018, Ilmoniemi2019}
\sntxt{.\sout{, but}
On the other hand,} we know very little about exactly \sntxt{\sout{what} which} types of neural activity that cause even the most well-known characteristics of the EEG signal, such as different types of oscillations (e.g., alpha, beta, and gamma waves) \sntxt{ and specific EEG-shapes in response to sensory stimuli \sout{, as well as the shape of the signal in response to sensory stimuli}} (event-related potentials, ERPs) \citep{COHEN2017}. \sntxt{\sout{However, we know that t} T}hese different \sntxt{EEG} characteristics \sntxt{\sout{of the EEG signal change in predictable ways as a result of} are affected by various brain conditions, such as sleep \sntxt{\sout{or} and} attention \citep{Klimesch1998, Palva2011, Siegel2012}, and by} brain disorders \sntxt{\sout{such as} including} epilepsy \sntxt{\sout{or} and} schizophrenia \citep{Niedermeyer2003, Light2013, Freestone2015, MAKI2019} \sntxt{\sout{or as a result of the condition of the brain}\citep{Klimesch1998, Palva2011, Siegel2012}}. In other words, a better \sntxt{insight into \sout{understanding of what aspects of the brain activity that are reflected in measured EEG signals could be of great help, both} how different types of brain activity is reflected in EEG signals is crucial} to understand \sntxt{\sout{what is going on in}} the human brain and to cure brain \sntxt{diseases \sout{disorders}} \citep{Uhlirova2016, COHEN2017, MAKI2019}.

The reason \sntxt{\sout{for our lack of} why we lack} understanding of the neural origin of \sntxt{\sout{the}} EEG signal\sntxt{s} is multifaceted\sntxt{.\sout{, but a} A}mong the main causes are (i) \sntxt{strong ethical constraints on invasive human brain measurements and (ii)} the high number of neurons that contribute to the signal\sntxt{\sout{, and (ii) strong ethical constraints on which tools that can be used to investigate the human brain.}} However, in recent years there have been major advances in several relevant branches of neuroscience\sntxt{, meaning that \sout{means}} a better understanding of the EEG signal may now be within reach \citep{Uhlirova2016, COHEN2017}.

\sntxt{\sout{For example, there has been a} To bypass challenge (i), we look to the} rapid development in \sntxt{the} technology and \sntxt{\sout{the}} methods used to study neural activity in lab animals\sntxt{.\sout{, and t} T}he possibility to control and manipulate neural activity, while simultaneously recording both intracranial signals like the local field potential (LFP) \citep{Einevoll2007, Blomquist2009} and extracranial \sntxt{\sout{non-invasive}} signals like the EEG \citep{BRUYNS2017}, can be expected to make important contributions to our understanding of non-invasive \sntxt{measurements of human brain activity \sout{brain signals measured in humans}} \citep{SILVA2013, Uhlirova2016, COHEN2017, Pesaran2018}. 
Futhermore, detailed biophysical modeling of neural activity has become an important tool for understanding intracranial LFP measurements \citep{EINEVOLL2013REVIEW, Pesaran2018} \sntxt{\sout{, and g} G}iven that \sntxt{\sout{the}} EEG is expected to reflect the same basic process \sntxt{as LFP}, that is, large numbers of synaptic input to geometrically aligned pyramidal cells \citep{NUNEZ2006, Pesaran2018}, it seems likely that detailed biophysical modeling can also help shed light on the neural origin of EEG signals.

\sntxt{\sout{ There are, however, some obstacles to overcome.
Firstly,} As indicated in challenge (ii),} EEG signals are expected to reflect the activity of much larger neural populations than the LFPs, making the simulations \sntxt{\sout{very}} computationally demanding. Biophysically detailed large-scale simulations of neural networks have, however, been gaining substantial momentum in recent years, thanks to large ongoing neuroscience initiatives like the Allen Institute for Brain Science, and the Human Brain Project \citep{EINEVOLL2019}. The possibility to calculate EEG signals from such existing and future large-scale biophysically detailed neural simulations could lead to valuable insights into the neural origin of the EEG.

\sntxt{\sout{Secondly} Another complicating aspect of EEG modeling, is that these predictions \sout{, modeling EEG signals}} in general require a head model to account for the widely different electrical conductivities of the brain, cerebrospinal fluid (CSF), skull and scalp \citep{NUNEZ2006, Ilmoniemi2019}. While many such head models exist, they tend to take current dipoles as input \citep{NUNEZ2006, Pesaran2018}, instead of the transmembrane currents that are available from biophysical neural simulations and that form the basis for modeling LFPs \citep{EINEVOLL2013BOOKCHAPTER}. 

%Would like to link EEG signals to animal LFP (where we know a lot more and can do a lot more), but this requires that we extend existing framework for LFP calc.

%However, while it is widely assumed that LFP and EEG signals reflects the same basic process, that is, large numbers of synaptic input to geometrically aligned pyramidal cells \citep{NUNEZ2006, Pesaran2018}, there are significant differences: Number of neurons, head models, current dipoles

%Important advances has also been made on the modeling side there has been great development in the tools needed \citep{TVB, HAGEN2018, NEYMOTIN2020} and in available computing power \citep{EINEVOLL2019}.

Here, we introduce an approach for automated reduction of arbitrary biophysically detailed simulated neural activity to current dipoles, and verify its applicability for \sntxt{\sout{calculation of}} EEG signal \sntxt{calculation}s. Further, we demonstrate how the approach \sntxt{\sout{can be} is} useful for investigating the origin of EEG signals, \sntxt{\sout{and} before explaining} how \sntxt{\sout{it} our methods} can be \sntxt{\sout{used with} applied for} pre-existing large-scale network models. Finally, we show how \sntxt{\sout{the}} current dipoles can be combined with realistic and complex head models, which enables simulation of EEG signals with \sntxt{\sout{an}} unprecedented biophysical detail.  


% Torbjørn's first draft:
...
%Despite EEG's long history and important role in neuroscience, we still know surprisingly little about the neural origin of the signal \citep{COHEN2017}. We have a relatively good understanding of the biophysical origin of the EEG signal: it mainly originates from large numbers synaptic inputs to cortical pyramidal cell populations \citep{NUNEZ2006, SILVA2013, Pesaran2018, Ilmoniemi2019}, but we know very little about exactly what types of neural activity that cause even the most well-known characteristics of the EEG signal, such as different types of oscillations (e.g., alpha, beta, and gamma waves), as well as the shape of the signal in response to sensory stimuli (event-related potentials, ERPs) \citep{COHEN2017}. However, we know that these different characteristics of the EEG signal change in predictable ways as a result of brain disorders such as epilepsy or schizophrenia \citep{Niedermeyer2003, Light2013, Freestone2015, MAKI2019} or as a result of the condition of the brain, such as sleep or attention \citep{Klimesch1998, Palva2011, Siegel2012}. In other words, a better understanding of what aspects of the brain activity that is reflected in measured EEG signals could be of great help, both to understand what is going on in the brain and to cure brain disorders \citep{Uhlirova2016, COHEN2017, MAKI2019}.

%The reason for our lack of understanding of the neural origin of the EEG signal is multifaceted, but among the main causes are (i) the high number of neurons that contribute to the signal, and (ii) strong ethical constraints on which tools can be used to investigate the human brain. However, in recent years there have been major advances in several relevant branches of neuroscience that means a better understanding of the EEG signal may now be within reach \citep{Uhlirova2016, COHEN2017}. 
%For example, there has been a rapid development in technology and the methods used to study neural activity in lab animals, and the possibility to control and manipulate neural activity, while simultaneously recording both intracranial signals like the local field potential (LFP) \citep{Einevoll2007, Blomquist2009}, and extracranial non-invasive signals like the EEG \citep{BRUYNS2017} can be expected to make important contributions our understanding of non-invasive brain signals measured in humans \citep{SILVA2013, Uhlirova2016, COHEN2017, Pesaran2018}.
%Futhermore, detailed biophysical modeling of neural activity has become an important tool for understanding intracranial LFP measurements \citep{EINEVOLL2013REVIEW, Pesaran2018}, and given that the EEG is expected to reflect the same basic process, that is, large numbers of synaptic input to geometrically aligned pyramidal cells \citep{NUNEZ2006, Pesaran2018}, it seems likely that detailed biophysical modeling can also help shed light on the neural origin of EEG signals. 

%There are however some obstacles to overcome. 
%Firstly, EEG signals are expected to reflect the activity of much larger neural populations than the LFP, making the simulations very computationally demanding. Biophysically detailed large-scale simulations of neural networks have, however, been gaining substantial momentum in recent years, thanks to large ongoing neuroscience initiatives like the Allen Institute for Brain Science, and the Human Brain Project \citep{EINEVOLL2019}. The possibility to calculate EEG signals from such existing and future large-scale biophysically detailed neural simulations could lead to valuable insights into the neural origin of the EEG.

%Secondly, modeling EEG signals in general requires a head model to account for the widely different electrical conductivity of the brain, cerebrospinal fluid (CSF), skull and scalp \citep{NUNEZ2006, Ilmoniemi2019}. While many such head models exist, they tend to take current dipoles as input \citep{NUNEZ2006, Pesaran2018}, instead of the transmembrane currents that are available from biophysical neural simulations and that form the basis for modeling LFPs \citep{EINEVOLL2013BOOKCHAPTER}. 

%Here we introduce an approach for automated reduction of arbitrary biophysically detailed simulated neural activity to current dipoles, and verify its applicability for calculation of EEG signals. Further, we demonstrate how the approach can be useful for investigating the origin of EEG signals, and how it can be used with pre-existing large-scale network models. Finally, we show how the current dipoles can be combined with realistic and complex head models, which enables simulation of EEG signals with an unprecedented biophysical detail.  
...
%\snnote{smart!}
In neural tissue, the current monopole contribution is zero due to current conservation, since the transmembrane currents sum to zero at all times \citep{Koch1999, PETTERSEN2012}. Further, the quadrupole, octopole and higher order terms are negligible to the current dipole contribution when $R$ is sufficiently large.  Therefore, the extracellular potential from a neuron simulation can be estimated with the second term of the current multipole expansion; an approximation known as the \textit{current dipole approximation} \citep{PETTERSEN&EINEVOLL2008,PETTERSEN2014,NUNEZ2006}:
...
For results shown in Fig.~\ref{fig:dipole_field}-\ref{fig:ca_spike}, synaptic input was modeled as conductance-based, exponential synapses (\texttt{Exp2Syn} in \texttt{NEURON}). The rise time constant was set to 1 ms and the decay time constant was 3 ms, synaptic reversal potential was 0 mV and the synaptic weight was set to 0.002, unless otherwise specified. %\tvnnote{Gjelder ikke hybrid modellen. Kanskje gi figurnummer?}
...
We quantified the model dissimilarities by looking at the relative error, and for a chosen distal synaptic input the relative error was 40.0\% and 1.06\% at the position of the ECoG and EEG electrodes respectively (Fig.~\ref{fig:compare_multi_single_dipole}\textbf{C}, green line). For a proximal synaptic input we observed a relative error of 76.1$\%$ at the ECoG position, and 7.61$\%$ at the EEG position (Fig.~\ref{fig:compare_multi_single_dipole}\textbf{C}, purple line). \sntxt{Inserting a single strong synaptic current (synaptic weight 0.05) into the soma, resulting in a somatic spike, gave relative errors of 34.7$\%$ and 0.967$\%$ for the computed ECoG and EEG signals, respectively (results not shown).}
In other words, the single-dipole approximation can result in substantial errors at the position of the ECoG electrodes, but appears justified at the position of the EEG electrodes for some synaptic locations.
...
So far, we have only considered \sntxt{\sout{the}} EEG contributions from single cells, but real EEG signals are expected to reflect the activity of hundreds of thousands to millions of cells \citep{NUNEZ2006, COHEN2017}. 
...
\snnote{Fantastisk! Jeg digger innholdet, og synes det er veldig fint skrevet! Har prøvd å gjøre det enda lettere å lese ved å fjerne noen repeterende ord og å dele opp noen litt lange setninger. Du får se hva du evt er enig i.}
\snnote{Klarer ikke helt å bestemme meg for hva som er lurt i starten her. Syntes det ble litt rart at vi "In this paper" [we have] "implemented the approach", så har prøvd å dele det opp uten å drepe flyten fullstendig.. Veldig åpen for andre forslag, altså!!}
%\subsection*{Summary}
In this paper, we have introduced an approach for reducing arbitrary simulated neural activity to single current dipoles (Fig.~\ref{fig:dipole_field})\sntxt{. Note that we have implemented all methods \sout{, and implemented the approach}} into the open-source python tool LFPy 2.0 \sntxt{\sout{ (note} and} that \sntxt{the} exact implementation presented here has already been used by \cite{HAGEN2018} and \cite{MAKI2019}\sntxt{\sout{ )}}. \sntxt{Here, }we verified that the approach was applicable for calculating EEG, but not ECoG signals (Fig.~\ref{fig:compare_multi_single_dipole}-\ref{fig:eeg_compare_cell_types}), and \sntxt{\sout{showed} gave} examples of how \sntxt{reducing neural activity to a single dipole \sout{this approach}} can be a powerful tool for investigating and understanding single-cell EEG contributions (Fig.~\ref{fig:eeg_compare_cell_types}-\ref{fig:ca_spike}). Furthermore, we demonstrated that the presented approach could \sntxt{\sout{be}} easily \sntxt{be} integrated with existing large-scale simulations of neural activity\sntxt{. \sout{, and how it can be used to construct} Moreover, it was shown how single dipoles are useful for constructing} compact representations of the EEG contributions from entire neural populations, \sntxt{\sout{while} with methods} still firmly grounded in the underlying biophysics (Fig.~\ref{fig:population}). Finally, we demonstrated how the simulated current dipoles, from single cells or large neural populations, can be directly inserted into complex head models for calculating more realistic EEG signals (Fig.~\ref{fig:compare_head_models}).
...
EEG signals have been an important part of neuroscience for a century, but still very little is known about the neural origin of the signal \citep{COHEN2017}. This work lays some of the foundation for obtaining a better understanding of the EEG signal, by allowing easy calculation of \sntxt{\sout{EEG signals} scalp potentials} from arbitrary simulated neural activity.
The presented formalism is well suited for modeling \sntxt{\sout{studies of}} \sntxt{EEG contributions \sout{the contribution to the EEG signal}} from \sntxt{\sout{different} various} potential neural origins, \sntxt{\sout{like} including} different cell types, different ion channels\sntxt{\sout{, or} and} different synaptic pathways.
In particular, \sntxt{\sout{notice that}} to \sntxt{\sout{judge} study} the \sntxt{\sout{contribution} effect} of
\snnote{litt for muntlig med "say"?}
\sntxt{\sout{, say,} for example} calcium spikes \citep{SUZUKI2017} or I$_{\rm h}$\sntxt{-?}currents on human EEG signals \citep{NESS2016, NESS2018, KALMBACH2018}, one only needs to understand how the z-component of the resulting population \sntxt{dipole \sout{current dipoles}} is affected\sntxt{\sout{, without needing the added complexity of}, which means disregarding the complexity added by} head models.
This decoupling of \sntxt{\sout{the }}current dipole moment\sntxt{\sout{s}} and head model\sntxt{\sout{s}} \sntxt{\sout{presents a simplified way of investigating} allows for easier investigation and improved understanding of} the origin of the EEG signal.
\sntxt{As illustrated in \cite{MAKI2019}, modeling EEG signals from simulated neural activity through calculation of the current dipole moment, will be an important brick when bridging the gap between individual genes and clinical EEG signal phenotypes.}\snnote{Passet dette inn? Tenkte det kanskje var fint å ha med?}

EEG measurements are often used for source localization, \sntxt{\sout{which aims} aiming} to identify the underlying cortical current dipoles \citep{NUNEZ2006, Gramfort2014, Ilmoniemi2019}. However, such reconstructed current dipoles are generic in the sense that they are typically not intended to represent specific neural populations. By allowing for calculation of current dipoles from cortical populations, \sntxt{\sout{this work} the work presented here} takes a step towards consolidating the, so far, mostly separate scientific disciplines of neural modeling and EEG data analysis (but see also \cite{NEYMOTIN2020}).
...
\sntxt{\sout{Arguably, we today} Today, we} have a reasonably good understanding of how single neurons operate, that is, how they respond to synaptic input, and how multitudes of \sntxt{\sout{such}} synaptic inputs combine to produce action potentials \citep{EINEVOLL2019}. Similarly, we \sntxt{\sout{have a reasonably good understanding of} can, to a high degree, explain} the measurement physics of the EEG signal, that is, how \sntxt{\sout{the}} neural currents affect \sntxt{\sout{the}} electric potentials recorded outside \sntxt{of} the head \citep{NUNEZ2006, COHEN2017}.
The challenge \sntxt{\sout{in} of} understanding EEG signals is therefore closely related to the greatest challenge in modern neuroscience: understanding neural networks. \sntxt{\sout{Understanding} Making sense of} such complicated dynamical systems typically requires \sntxt{\sout{some kind of}} computational modeling \citep{EINEVOLL2019}, \sntxt{\sout{however} but}, the complexity of neurons, and the complexity and size of the neural networks involved in even the simplest of cognitive tasks, makes this a daunting challenge. The steady increase in available computing power, in combination with the ever-increasing knowledge on synaptic connectivity patterns is, however, making this approach \sntxt{\sout{increasingly} more and more} attractive \citep{Reimann2013, Egger2014, MARKRAM2015, HAGEN2016, Gratiy2018, Arkhipov2018, Reimann2019, BILLEH2019}\sntxt{\sout{, and}: Today, there are} several ongoing research projects \sntxt{\sout{are}} pursuing such modeling efforts, like the Allen Institute for Brain Science and the Human Brain Project. While \sntxt{\sout{such}} biophysically detailed, large-scale neural simulation\sntxt{s are} still in \sntxt{their\sout{its}} infancy, we expect \sntxt{\sout{it} these simulations} to become an increasingly important research tool in neuroscience \citep{EINEVOLL2019}
\sntxt{.\sout{, and by enabling simulation of EEGs with realistic head models from such simulations, we hope such approaches can help shed light on the neural origin of the EEG in the coming years.}}
%........................
%\sntxt{We hope that enabling EEG simulations combining biophysically detailed neuron simulations with realistic head models, can help shedding light on the neural origin of the EEG in the coming years.}
\sntxt{In the coming years, we hope that our enabling of EEG simulations combining biophysically detailed neuron simulations with realistic head models will help shedding light on the neural origin of EEG.}
...
Note that while we here used LFPy 2.0 \citep{HAGEN2018, HAGEN2019}, a python interface to NEURON \citep{CARNEVALE2006}, \sntxt{\sout{the}} calculation of \sntxt{\sout{the}} current dipole moments can easily be implemented into any framework where the transmembrane currents are available, through the simple formula given in eq.~(\ref{eq:dip_sum_trans}). Calculation\sntxt{\sout{s}} of current dipole moments from morphologically complex cell models \sntxt{\sout{have} has} been pursued before, for example to study \sntxt{EEG and MEG contributions from spiking single cells \sout{the contribution of spikes from single cells to EEG and MEG}} \citep{Murakami2006}, or to study \sntxt{how \sout{the effect of the}} synaptic input location \sntxt{affects the current dipole}\citep{LINDEN2010, AHLFORS2015}.
...
Important work \sntxt{\sout{in interpreting EEGs} on EEG interpretation} in terms of the underlying neural activity has \sntxt{previously} been done \sntxt{\sout{previously}} through use of "minimally sufficient" biophysical models, see for example \cite{Murakami2002, Murakami2003, Jones2007, Jones2009, Sliva2018, NEYMOTIN2020}. Here, "minimally sufficient" means that the cell models only had the minimum needed spatial structure (point neuron can not produce current dipole moments), only considered a few cell types, and employed simple synaptic connection rules. In particular, the Human Neocortical Neurosolver (HNN) \citep{NEYMOTIN2020} \sntxt{\sout{is designed to allow} enables} researchers to link measured EEG or MEG recordings to neural activity \sntxt{\sout{simulated}} through a pre-defined canonical neocortical column template network. HNN comes with an interactive GUI, designed for users with little or no experience in computational modeling, and might therefore be an appropriate choice for researchers seeking to gain a better understanding of their EEG/MEG data. 
However, while the use of such minimally sufficient models allows for quick and direct comparison between simulated and recorded EEG signals, it is not (presently) compatible with simulating EEGs or MEGs from biophysically detailed single cell- or network models, constructed from detailed experimental data \citep{Reimann2013, Egger2014, MARKRAM2015, HAGEN2016, Gratiy2018, Arkhipov2018, BILLEH2019}.  
...
A more high-level approach to simulate MEG/EEG signals from the underlying neural activity has been pursued through neural field or neural mass models \citep{Jirsa2002, David2003, Coombes2006, Deco2008, Bojak2010, Ritter2013}, which aim to model the evolution of coarse-grained variables such as the mean membrane potential or the firing rate of \sntxt{neuron populations \sout{populations of neurons}}. Such coarse-graining drastically reduces the number of parameters and the computational burden of the simulation, and can be used to study the interplay among entire brain regions, and indeed run whole-brain simulations. 
The Virtual Brain (TVB) is an excellent example of a software for whole-brain network simulations \citep{TVB, Sanz-Leon2015, Ritter2013}, where detailed and potentially personalized head models can be \sntxt{\sout{used and}} combined with \sntxt{tractography-based methods identifying} the connectivity between brain regions \sntxt{\sout{as identified by tractography based methods}} \citep{TVB}. To calculate measurement modalities like MEG and/or EEG signals from neural field or neural mass models, it is typically assumed that the population current dipole moments are roughly proportional to, for example, the average excitatory membrane potential \citep{Bojak2010, Ritter2013} \sntxt{.\sout{, and t} Further, EEGs can be calculated from t}he resulting current dipole moments \sntxt{\sout{can be used}} in combination with head models as presented in this paper, or through other softwares or techniques \citep{Gramfort2014}\sntxt{\sout{ to calculate EEG signals}}.

Note that \sntxt{the above-mentioned work \sout{this}} suggests an intriguing future development, where one could, with high biological realism, calculate the average post-synaptic current dipole moment of a given neural population\sntxt{\sout{, i} I}n practice\sntxt{, this would involve \sout{done through}} simultaneously activating all outgoing synapses from a simulated population, and recording the total current dipole moment of the response (the kernel) \citep{HAGEN2016}. Convolving this kernel with the population firing rate would then provide a good approximation of the current dipole moment caused by the given neural population, and the EEG contribution could subsequently be calculated. This so-called kernel method has previously successfully been applied to the LFP \citep{HAGEN2016, Skaar2020, Telenczuk2020}, and it could provide a way to substantially increase the accuracy of LFP, EEG and MEG predictions from high-level simulations of neural activity.  
...
%\subsection*{Outlook}
While there are many examples of \sntxt{\sout{where}} detailed biophysical modeling of neural activity \sntxt{\sout{has helped} improving} interpretation of measured intracranial extracellular potentials in lab animals \citep{Einevoll2007, Blomquist2009, McColgan2017, Luo2018, Chatzikalymniou2018, Telenczuk2019}, much less has been done for human EEG signals\sntxt{.\sout{, because} This is a natural cause of} studies of the healthy human brain \sntxt{\sout{is}} necessarily \sntxt{being} limited to non-invasive technologies \citep{SILVA2013, Uhlirova2016, COHEN2017}.
However, given all the valuable insights that could be gained from an increased understanding of non-invasive measurements of neural activity in humans, an important challenge in modern neuroscience is to build on the mechanistic insights from \sntxt{\sout{such}} animal studies \sntxt{\sout{and use them for understanding non-invasive signals in humans}} \citep{SILVA2013, Uhlirova2016, COHEN2017, EINEVOLL2019, MAKI2019}.
