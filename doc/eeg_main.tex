...
\sntxt{\sout{We confirmed that the reason for this sudden drop was that} Here,} synaptic inputs slightly distal to 400~$\si{\um}$ away from soma resulted in the majority of \sntxt{the} return currents \sntxt{\sout{being} escaping the cell} below the synaptic input (closer to the soma). This gave a distinctly dipole-like source/sink distribution, and thereby low relative errors ($\sim$0.4\%). Synaptic inputs slightly proximal to 400~$\si{\um}$ away from soma instead resulted in almost balanced return currents above and below the synaptic input. This gave a multipole-like source/sink distribution, and thereby larger relative errors ($\sim$7\%). Note, however,
...
\newtxt{We here chose to focus on the scenario with transient thalamocortical input for mimicking event-related potentials (ERPs)\sntxt{\sout{, but instead f} However,} focusing on different neural oscillations (brain waves) or spontaneous activity would have served equally well for our purposes.}
...
To test this simplified approach, we combined the current dipole moments with the four-sphere head model (Fig.~\ref{fig:population}\textbf{E}). We \newtxt{calculated the EEG signal by the simplified approach, that is, from one time-dependent array, $p_z$, for each pyramidal cell population, located in the center of the given population. We then compared this \sntxt{simplified approach} with the original approach, that is, the sum of EEG contributions from all $\sim$80 000 cells at their respective positions. The simplified approach gave a maximum relative error of 1.07~\% (maximum absolute difference normalized by maximum value of EEG from original approach) (Fig.~\ref{fig:population}\textbf{F} black versus red dashed line). This implies that the EEG signal from the simulated cortical activity can be \newtxt{nearly} fully represented by a single time-dependent variable for each pyramidal cell population. Further, summing these population dipoles into one single dipole, and locating it in the center of the population column at 1~mm depth, instead gave a maximum relative error of 6.62~\%. This highlights that the exact position of different pyramidal cell populations are relatively unimportant for shaping the EEG signal.}
