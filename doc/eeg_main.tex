...
\section*{Higlights}
\begin{itemize}
\item Current dipoles are computed from simulated neuron and network activity
...
\tvnnote{1 uA * um ~-> 1 nV, so 10000 Ca spikes gives ~10 uV EEG.} 
...
\textbf{D:} Contour lines of extracellular potential around neuron at a snapshot in time (T=32.2~ms) during the somatic spike in \textbf{B1}.
\textbf{E:} Contour lines of extracellular potential around neuron at a snapshot in time (T=36.0~ms) during the calcium spike in \textbf{C1}.
		\gen{Igjen burde vel de synaptiske str{\o}mmene bli spesifisert, selv om de ikke er hovedpoenget her?}
	}
	\label{fig:ca_spike}
\end{figure}
...
\subsection{\tvntxt{Application of current dipoles}}
