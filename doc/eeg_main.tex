...
An even more high-level approach to simulate MEG/EEG signals have been pursued through neural field or neural mass models \citep{David2003, Coombes2006}, which aims to model neural activity at the population level

VB is a software for full brain network simulations using detailed connectivity between brain regions by long-range neural fiber tracts as identified by tractography based methods \citep{TVB}, and can therefore use personalized head models.
...
In TVB, our main interest lies in using the mesoscopic laws governing the behavior of neural populations and uncovering the laws driving the processes on the macroscopic brain network scale. The biophysical mechanisms available to microscopic single neuron approaches are absorbed in the mean field parameters on the mesoscopic scale and are not available for exploration other than through variation of the mean field parameters themselves. As a consequence, TVB represents a neuroinformatics tool that is designed to aid in the exploration of large-scale network mecha- nisms of brain functioning [see Ritter et al. (2013) for an example of modeling with TVB].
...
"
TVB thus represents a unique tool to systematically investigate the dynamics of the brain, emphasizing its
large-scale network nature and moving away from the study
of isolated regional responses, thereby considering the function of each region in terms of the interplay among brain regions."
