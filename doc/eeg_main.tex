...
We introduce our approach for detailed biophysical modelling of ECoG, EEG and MEG signals by considering a single synaptic input to a biophysically detailed cell model. 

\subsection{At sufficiently large distances, extracellular potentials become dipolar}\label{subsec:cb_db_comp_inf}
\tvnnote{I rewrote this section to emphasize that it is an illustration, and removed some technical details that I think wasn't needed here.}
...
For illustration we first assume that the cell is positioned in an infinite homogeneous medium, in which case the extracellular potential can be easily calculated from either the transmembrane currents (eq.~\ref{eq:point_source}) or from the multi-dipole or single-dipole approximation (eq.~\ref{eq:dipole}). Very close to the cell the extracellular potential will strongly depend on the exact distribution of transmembrane currents across the cellular morphology, and it will therefore typically not have the shape of a single dipole (Fig.~\ref{fig:dipole_field} \textbf{D,E} versus \textbf{F}). However, since the extracellular potential
...
The ECoG and EEG signals are strongly affected by the very different conductivity of the CSF, skull and scalp \citep{NUNEZ2006}, and therefore, to test the applicability of the single-dipole approximation for computing ECoG and EEG signals, we used the four-sphere head-model \citep{NAESS2017, HAGEN2018, HAGEN2019}. 
...
We also observed only small differences between the multi-dipole approach and the single-dipole approximation for the distal synaptic input, while for the proximal synaptic input we observed a substantial difference directly above the cell model, that was substantially decreased along the path towards the top of the head (Fig.~\ref{fig:compare_multi_single_dipole}\textbf{B}). We quantified these differences by looking at the relative error, and found that for the distal synaptic input the relative error was 20.0(?) \% and 1.0(?) \% at the position of the ECoG and EEG electrodes respectively, while for the proximal synaptic input the relative error was 80.0(?) \% and 20.0(?) \% at the position of the ECoG and EEG electrodes respectively (Fig.~\ref{fig:compare_multi_single_dipole}\textbf{C})
...
Pyramidal cells have a preferred orientation along the depth axis of the cortex (here the $z$-axis), and the direction of the current dipole moment can be expected to align with this axis since
radial symmetry will tend to make the orthogonal components cancel \citep{HAGEN2018}. 
...
receiving a wave of 1'000 excitatory synaptic inputs that were restricted to the uppermost 250(?)~$\si{\um}$ of the cell (time=?; name=?, Fig.~\ref{fig:eeg_compare_cell_types}\textbf{B}), we observed a negative deviation of the $p_z$-component (Fig.~\ref{fig:eeg_compare_cell_types}\textbf{C}). For basal synaptic input, the polarity of $p_z$ was instead positive, but of slightly lower amplitude than for apical input. 
This can be explained by more local return currents


that were relatively similar in shape, but had a somewhat higher amplitude for the larger \tvnnote{How does the diameters of the main apical dendrites compare?} layer 5 model

We used two different cortical pyramidal cell models, that is, the previously used passive human layer 2/3 cell from \cite{EYAL2016}, and in addition a rat layer 5 cell model from \cite{HAY2011} with 10 different active conductances and capability for both somatic and dendritic spikes (but note that for now, all synaptic input was subthreshold) \tvnnote{Need good way to refer to the specific morphology in Figure 3}. 
%For a wave of 1'000 excitatory synaptic inputs that were restricted to the uppermost 250(?)~$\si{\um}$ of the cells (time=?; name=?, Fig.~\ref{fig:eeg_compare_cell_types}\textbf{B}), we observed negative deviation of the $p_z$-component, that were relatively similar in shape, but had a somewhat higher amplitude for the larger \tvnnote{How does the diameters of the main apical dendrites compare?} layer 5 model Fig.~\ref{fig:eeg_compare_cell_types}\textbf{C}. For basal synaptic input, the polarity of $p_z$ was instead positive, but of slightly lower amplitude than for apical input. 
%This can be explained by more local return currents


%The models received different waves of synaptic input, the first was excitatory input to the distal apical dendrite, the second to the basal dendrite, the third uniformly distributed across the cell, and the fourth

%(and therefore the polarity of the EEG signal) will be determined by the synaptic input, with apical and basal synaptic input giving current dipoles of opposite orientation (cite). 

%For all cases the components of the current dipole moment ($\mathbf{p}$) that was orthogonal ($p_x$ and $p_y$) to the depth axis of cortex ($z$) was negligible compared to the $p_z$-component (Fig.~\ref{fig:eeg_compare_cell_types}\textbf{C}), as expected from radial symmetry. This means that we can describe the single-cell EEG contribution fully in terms of a single time-dependent array, instead of in terms of the same number of current sources/sinks as the number of compartments (typically ~ 1000 for a large pyramidal cell).
