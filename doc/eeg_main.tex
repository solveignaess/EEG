...
These signals are known to originate from \sntxt{\sout{the}} transmembrane currents, typically described in terms of current dipoles, which are associated with cortical neural activity.
\snnote{Alternativ til setning over: These signals are known to originate from cortical neural activity, typically described in terms of current dipoles.}
...
\snnote{Forslag: bytte ut "framework" her med "method", for å spare "framework" til siste setning, der vi snakker om modellering av hele prosessen fra nervecelleaktivitet til EEG/MEG?}
Here\sntxt{,} we present a \sntxt{\sout{framework} method} for reducing arbitrary simulated neural activity to simple current dipoles.
\gen{Jeg synes "single" er bedre enn "simple" (som i grunn ikke sier saa mye).}
We find that the \sntxt{\sout{framework} method} is applicable for calculating extracranial signals, but less suited for calculating intracranial ECoG signals. We demonstrate that this approach can serve as a powerful tool for investigating the neural origin of EEG/MEG signals. This is done through example studies of the putative EEG contribution from calcium spikes, 
\gen{Kan vel ogsaa nevne det foerste eksemplet med single-neuron EEG contribution?} and from calculating EEG signals from biophysically detailed large-scale neural network simulations.
\gen{Kanskje bedre aa droppe "biophysically detailed" her?}  We also demonstrate how the simulated current dipoles can be used directly in combination with detailed head models, allowing for simulated EEG signals with an unprecedented level of biophysical details. 
...
\item Extracted current dipoles \gex{allow for accurate} computation of EEG and MEG signals in simplified and detailed head models
\item  \gex{Current-diplole approximation generally not suitable for accurate calculations of ECoG signals} 
...
\tvntxt{Note that the clear separation between calculation of current dipoles and the corresponding EEG is equally valid for magnetoencephalography (MEG) signals. While we here focus mostly on EEG, the presented approach for calculating current dipoles from neural activity is equally valid for MEG signals, through \gex{use of an} appropriate forward model \citep{Ilmoniemi2019}.}
...
\begin{table*}[ht]
  ...
  \gen{Kanskje ikke noedvendig aa nevne enheter i caption naar det staar i tabellen, men det skader vel ikke.}
  \label{tab:4sphere}
\end{table*}
...
We introduce an approach for modeling \tvntxt{electroencephalography (EEG) and magnetoencephalography (MEG)} 
signals from detailed biophysical multicompartment cell models.  \gex{The approach involves two steps: First, current dipole moments are extracted from activity
...
For illustration, we first consider EEG signals stemming from single synaptic input onto single neurons in an infinite homogeneous head model, before moving on to a simple, generic head model. Finally, we study EEGs from \tvntxt{large-scale simulated network} activity, also applying a realistic head model.}  \gen{Tok bort ECoG her i introen siden det ikke gav saa gode resultater}
...
\sntxt{In summary, the single-dipole approximation can result in substantial errors at the position of the ECoG electrodes, but gives small errors at the position of the EEG electrodes for synaptic locations leading to strong EEG signals.}
...
\tvntxt{To compare the approaches, we computed the relative error, \gex{that is}, the absolute difference between the results from the two approaches, normalized by the maximum EEG magnitude computed with the multi-dipole approach.}
...
\begin{figure}[H]
	...
	\caption{\textbf{EEG signals and current dipole moment from three different cell types with various synaptic input}.
		...
		\gen{Ja det er OK, men da boer det refereres til ett av stedene her.}
...
We found that the case with the dendritic calcium spike \gex{now} had a $6.6$-fold larger maximum amplitude than the case with
...
\gen{$\si{\uA}\cdot\si{\um}$ ser litt rart ut med prikken i midten, hva med $\si{\uA}\,\si{\um}$}
...
In this paper, we have introduced an approach for reducing arbitrary simulated neural activity \gex{from biophysically detailed neuron models} to single current dipoles (Fig.~\ref{fig:dipole_field}). 
...
\gen{Kan dette avsnittet flyttes hit eller et annet sted? Jeg synes ikke det passet inn paa slutten av "outlook" hvor vi loeftet fram engler og basuner ...}
\snnote{Ja, det passer vel greit inn i applications, men da synes jeg kanskje vi burde endre underoverskriften tilbake igjen, siden LFPy har vel så mye med EEG å gjøre?}
\gen{Enig. Endret naa.}
\gex{
...
\gex{approximate} trick, since neurons require a spatial structure to be capable of producing electromagnetic signals \citep{EINEVOLL2013REVIEW}.
One such trick that has \sntxt{\sout{already}} been taken advantage of here is the hybrid scheme\sntxt{.\sout{, where} This two-step scheme involves} neural network activity \sntxt{\sout{is}} first \sntxt{being} simulated by point neurons, \sntxt{\sout{and} before} the resulting spike trains \sntxt{are} replayed onto multi-compartment neuron models for calculating LFP and EEG signals (sec.~\ref{subsec:populations}; Fig.~\ref{fig:population}). 

\sntxt{\sout{This} Further, the hybrid scheme} can \sntxt{\sout{further}} be generalized by using the so-called kernel method, which has previously \sntxt{\sout{successfully}} been \sntxt{successfully} applied to the LFP \citep{HAGEN2016, Skaar2020, Telenczuk2020}. In practice, this would \gex{be done in two steps: First,\sntxt{\sout{calculation of the kernels by}} simultaneously activating all outgoing synapses from a specific (presynaptic) simulated population, and recording the total current dipole moment of the response (the kernel) \citep{HAGEN2016}. Second, computing the EEG/MEG contribution stemming from this (presynaptic) population by convolving \sntxt{\sout{this} the} kernel with the population firing   
rate\sntxt{. Here, the firing rate would be} obtained separately in point neuron network models or firing-rate models. In this way\sntxt{,} the basic biophysics of EEG and MEG signal\sntxt{s} from
synaptic activation of multi-compartment neuron models \sntxt{\sout{are} is} included\sntxt{, avoiding, however, computationally heavy multicompartmental modeling of spiking dynamics \sout{without relying on such models to model the spiking dynamics.}}  
...
\gen{Nei, ikke noedvendig synes jeg. Stroek ogsaa den siste setningen.}
...
Here, "minimally sufficient" means that the cell models only had \gex{minimally needed multi-compartment} spatial structure (\gex{point neurons cannot} produce current dipole moments), only considered a few cell types, and employed simple synaptic connection rules.
...
This suggests an intriguing future development, where one could apply the \gex{above\sntxt{-}mentioned kernel method based on biophysically detailed neuron models} to substantially increase the accuracy of LFP, EEG and MEG predictions from high-level large-scale simulations of neural activity. 
...
\gen{Synes de passer bra, men jeg foreslaar at vi i hovedsak nevner EEG og MEG i samme aandedrag. De viktigste bidragene i vaart arbeid gjelder jo for begge signaler.}

\gex{EEG and, later, MEG signals have been an important part of neuroscience for a long time}, but still very little is known about the neural origin of the signal\sntxt{s} \citep{COHEN2017}. \gex{A better understanding of these signals could lead to important discoveries about how the brain works \citep{SILVA2013, Uhlirova2016, Pesaran2018, Ilmoniemi2019}, and provide new insights into mental disorders \citep{MAKI2019, Sahin2019}.}
This work lays some of the foundation for obtaining a better understanding of the \gex{EEG/MEG recordings, by allowing easy calculation of the signals from arbitrary neural activity.}
The presented formalism is well suited for modeling \gex{EEG/MEG} contributions from various potential neural origins,  including different cell types, different ion channels and different synaptic pathways. 
...
\gex{EEG/MEG} measurements are often used for source localization, aiming to identify the underlying cortical current dipoles \citep{NUNEZ2006, Gramfort2014, Ilmoniemi2019}. However, such reconstructed current dipoles are generic in the sense that they are typically not intended to represent specific neural populations. By allowing for calculation of current dipoles from cortical populations, the work presented here takes a step towards consolidating the, so far, mostly separate scientific disciplines of neural modeling and \gex{EEG/MEG} data analysis (but see also \cite{NEYMOTIN2020}).

While there are many examples of detailed biophysical modeling of neural activity improving interpretation of measured intracranial extracellular potentials in lab animals \citep{Einevoll2007, Blomquist2009, McColgan2017, Luo2018, Chatzikalymniou2018, Telenczuk2019}, much less has been done for human EEG signals. This is a natural \gex{given that studies of healthy human brains necessarily are} limited to non-invasive technologies \citep{SILVA2013, Uhlirova2016, COHEN2017}.
...
The approach for calculating EEG/MEG signals in this paper should therefore ideally be used in combination with animal studies simultaneously measuring multisite laminar LFP (and MUA) signals within cortex, as well as \gex{EEG/MEG} signals (see for example \cite{BRUYNS2017}) \citep{COHEN2017}.

\snnote{I neste avsnitt har vi kun snakket om EEG. Disse tingene gjelder vel også for MEG?}
...
Similarly, we can, to a high degree, explain the measurement physics of \sntxt{\sout{the}} EEG\sntxt{/MEG \sout{signal}}, that is, how neural currents affect \sntxt{\sout{electric potentials} electromagnetic brain signals} recorded outside of the head \citep{NUNEZ2006, COHEN2017,Ilmoniemi2019}.
The challenge of understanding EEG\sntxt{/MEG} signals is therefore closely related to the greatest challenge in modern neuroscience: understanding neural networks. 
...
\sntxt{\sout{In the coming years, we \gex{believe} that \gex{the presently described method enabling} EEG simulations combining biophysically detailed neuron simulations with realistic head models, will help shedding light on the neural origin of EEG.
We \gex{thus hope} that the work \sntxt{\sout{potentially}} help us to take full advantage of this important brain signal in the future.}}

\snnote{Lurer på om slutten kunne vært hakket mindre ydmyk? Her er forsøk på å få inn en smule mer pønsj..}

\snnote{Forsøk 1:}
\sntxt{We believe that the presently described method will help shedding light on the neural origin of EEG/MEG, and help us take full advantage of these important brain signals in the future.}

\snnote{Forsøk 2:}
\sntxt{The presently described method enables EEG/MEG simulations combining detailed neural simulations with realistic head models. We believe that this approach will help shedding light on the neural origin of EEG/MEG signals, and help us take full advantage of these important brain signals in the future.}

\gen{Enig. Ydmykhet er for pingler :-) Jeg likte Forsoek 2 best.} 
