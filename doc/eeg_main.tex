...
Neural activity generates electric currents in the brain, which in turn create electromagnetic fields. In this section, we explain how \sntxt{we model} electric brain signals \sntxt{\sout{can be modeled}} in simple and more complex volume conductors.
...
\sntxt{\sout{, and can easily be applied making use of} and was applied using the Python package \texttt{LFPy 2.0} running NEURON under the hood} \citep{HAGEN2018,CARNEVALE2006}.
...
\snnote{Dette er jo egentlig ikke nytt, da, var jo allerede på plass i LFPy2.0-artikkelen, men det er kanskje greit å ha det der likevel?}
We have implemented both above-mentioned methods in LFPy2.0, see \texttt{cell.get\_multi\_current\_dipole\_moments()} and \texttt{cell.current\_dipole\_moment}.
...
We have found that the single-cell current dipole moment is applicable for calculating a neuron's EEG-contribution, and that thiss (Fig.~\ref{fig:eeg_compare_cell_types}-\ref{fig:ca_spike}). Furthermore, we demonstrated that the presented approach can be helpful in estimating the relativould be easily integrated with existing large-scale simulations of neural activity, and how it can be used to construct compact representations of the EEG- contribution of different cell types and different modes of synaptic input, in additig., different cell types, from somatic spikes, or fc.
\begin{itemize}
