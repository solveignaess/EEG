...
Here, we introduce an approach for automated reduction of arbitrary biophysically detailed simulated neural activity to current dipoles, and verify its applicability for EEG signal calculations. Further, we \tvntxt{\sout{demonstrate} show} how the approach is useful for investigating the origin of EEG signals, before \tvntxt{\sout{explaining} demonstrating} how our methods can be applied for pre-existing large-scale network models. Finally, we show how current dipoles can be combined with realistic and complex head models, which enables simulation of EEG signals with unprecedented biophysical detail.  
...
In particular, to study the effect of for example calcium spikes \citep{SUZUKI2017}, I$_{\rm h}$ currents \citep{NESS2016, NESS2018, KALMBACH2018}, \tvntxt{or individual gene expression on clinical EEG signal phenotypes \citep{MAKI2019}, }
