...
This can be further generalized by using the so-called kernel method, which has previously successfully been applied to the LFP \citep{HAGEN2016, Skaar2020, Telenczuk2020}. In practice, this would \gex{be done in two steps: First, calculation of the kernels by simultaneously activating all outgoing synapses from a specific (presynaptic) simulated population, and recording the total current dipole moment of the response (the kernel) \citep{HAGEN2016}. Second, computing the EEG/MEG contribution stemming from this (presynaptic) population by convolving this kernel with the population firing   
rate obtained separately in point neuron network models or firing-rate models. In this way the basic biophysics of EEG and MEG signal from
synaptic activation of multi-compartment neuron models are included without relying on such models to model the spiking dynamics.  
...
This suggests an intriguing future development, where one could apply the \gex{abovementioned kernel method based on biophysically detailed neuron models} to substantially increase the accuracy of LFP, EEG and MEG predictions from high-level large-scale simulations of neural activity. 
...
\gex{EEG and, later, MEG signals have been an important part of neuroscience for a long time}, but still very little is known about the neural origin of the signal \citep{COHEN2017}. \gex{A better understanding of these signals could lead to important discoveries about how the brain works \citep{SILVA2013, Uhlirova2016, Pesaran2018, Ilmoniemi2019}, and provide new insights into mental disorders \citep{MAKI2019, Sahin2019}.}
...
In the coming years, we \gex{believe} that \gex{the presently described method enabling} EEG simulations combining biophysically detailed neuron simulations with realistic head models, will help shedding light on the neural origin of EEG.
We \gex{thus hope} that the work potentially help us to take full advantage of this important brain signal in the future.
