...
%There are several ongoing large-scale modelling efforts that seek to simulate neural activity at different levels of biological detail. 
Biophysically detailed modelling of large populations is still in its infancy \citep{EINEVOLL2019} and typically include ``only'' a few tens of thousands of biophysically detailed cells which are executed on large supercomputers \citep{MARKRAM2015, BILLEH2019}, while networks of point neurons are regularly used to simulate millions of cells (cite). 





To investigate the origin of the EEG signal from large populations of neurons, we used the hybrid scheme \citep{HAGEN2016}, where the network activity is first simulated in a highly computationally efficient manner with point neurons in NEST \citep{NEST} and the spiking activity of each neuron saved to file. Afterwards, the spikes are used as synaptic input to corresponding biophysically detailed multicompartment neurons such that the original connectivity structure of the network is preserved \citep{HAGEN2016}. 

We used the large-scale point-neuron cortical microcircuit model from \cite{POTJANS2014, HAGEN2016}, 

in combination with the hybrid scheme for calculating LFPs and EEGs \citep{HAGEN2016}. In the hybrid scheme ,
 (Fig.~\ref{fig:population}\textbf{A})
and synaptic input to the neurons is saved and used as synaptic input to corresponding biophysically detailed multicompartment neurons (Fig.~\ref{fig:population}\textbf{B}).


Breifly, this model has $\sim$80'000 neurons divided into 8 different cortical populations, one excitatory and one inhibitory across four layers (L2/3 - L6), and can exhibit a diverse set of spiking dynamics including different oscillations and asynchroneous irregular network states \citep{HAGEN2016, BRUNEL2000}. 
In the hybrid scheme The network activity was simulated for point neurons using NEST \citep{NEST},
 (Fig.~\ref{fig:population}\textbf{A})
 (Fig.~\ref{fig:population}\textbf{B}). This allows for calculation of LFPs, as was done by \cite{HAGEN2016}

from which the single-cell current dipole moment of each cell is obtained together with the single cell's contribution to the LFP and EEG.
