...
\begin{frontmatter}
	...
	\title{Biophysical modeling of \gex{the neural origin of EEG, ECoG and MEG} signals
	\snnote{Detailed biophysical modeling of EEG and MEG signals}
	\snnote{Bridging the gap between complex neural simulations and EEG and MEG signals with biophysical modeling}
	\snnote{Biophysical modeling of EEG and MEG signals based on detailed neural simulations}
...
\gen{Fin figur. Noen kommentarer: (1) Er litt usikker paa ordet "automatic" i "automatic reduction". Den er vel ikke mer "automatic" enn
andre "computational schemes" (med mindre vi lager en easy-to-use toolbox som brukerne kan bruke som en "black box")? (2) Er ogsaa litt usikker paa "realistic" i "realistic head models". Jeg synes "detailed" er bedre siden det er litt mindre "value judgement" i ordet. (3) Kanskje ogsaa "simplified" er bedre enn "generic"? (Og det staar "realsitic" istedenfor "realistic" et sted.)}
\snnote{møte: bytte ut realistic/ generic med complex}
...
\gen{Vi maa jobbe litt mer med disse}
\begin{itemize}
\item Arbitrary simulated neural activity can be automatically reduced to current dipoles
\gex{Current dipoles are computed from simulated neuron and network activity}  
\item Such current dipoles can be used with generic or realistic head models for calculation of EEG or MEG signals
\gex{Extracted current dipoles used in computation of EEG and MEG signals in simplified and detailed head models}  
\item \sout{Allows easy simulation of EEG signals from both single cells and large-scale neural networks}
\gen{One-liner about ECoG signal}
\item Gives clean way of studying the neural origin of EEG signals
\gex{Method provides biophysics-based link between neural activity and systems-level signals}
\end{itemize}
...
%\begin{itemize}
%\item Known: p-> EEG
%\item New = neural activity -> p 
%\item More complete forward model
%\item Inverse models relies on forward models
%\item Better forward models -> better inverse models
%\end{itemize}
%Inverse modeling, that is estimating the dipolar sources of EEG signals, . Improving forward models of EEGs can, therefore, also contribute to better inverse models. Through forward modeling since the link between current dipole moments and EEGs is well established. This link so-called inverse modeling of EEG signals  These dipoles mainly stem from synaptic input to cortical pyramidal cell populations.
%}
%
%Electroencephalography (EEG) is one of the most important non-invasive methods for studying human cognitive function and diagnosing brain diseases \citep{COHEN2017, Pesaran2018}.  Yet, we know surprisingly little about the neural origin of these electric scalp potentials \citep{COHEN2017}.  On the one hand, we have a relatively good understanding of the biophysics of EEGs\sntxt{: Well-established forward models gives the link between cortical current dipole moments and EEG signals. Until now, the possibility for computing these current dipole moments from detailed neural activity has not been taken full advantage of. }
%\snnote{Husker ikke hva vi konkluderte med her! Skulle vi ha med "synaptic inputs to cortical pyramidal cell populations" eller ikke?}
%
%
%\sout{
% that these signals \sntxt{are generated by cortical current dipole moments. Inverse modeling. } 
%
%mainly originate from large numbers of synaptic inputs to cortical pyramidal cell populations \citep{NUNEZ2006, SILVA2013, Pesaran2018, Ilmoniemi2019}.
%On the other hand, we know very little about exactly which types of neural activity that cause even the most well-studied characteristics of the EEG signal, such as different types of oscillations (e.g., alpha, beta, and gamma waves) and stereotyped EEG-shapes in response to sensory stimuli (event-related potentials, ERPs) \citep{COHEN2017}. Importantly, these different EEG characteristics are affected in predictable ways by various brain conditions, such as sleep and attention \citep{Klimesch1998, Palva2011, Siegel2012}, and by brain disorders including epilepsy and schizophrenia \citep{Niedermeyer2003, Light2013, Freestone2015, MAKI2019}. 
%In other words, a better insight into how different types of brain activity is reflected in EEG signals could help us in understanding the human brain and curing brain diseases  \citep{Uhlirova2016, COHEN2017, MAKI2019}.

%\snnote{Intro suggestion 2:}
%Electroencephalography (EEG) is one of the most important non-invasive methods for studying human cognitive function and diagnosing brain diseases \citep{COHEN2017, Pesaran2018}.  Yet, we know surprisingly little about the neural origin of these electric scalp potentials \citep{COHEN2017}: On the one hand, we have a relatively good understanding of the biophysics of EEGs, knowing that these signals mainly originate from large numbers of synaptic inputs to cortical pyramidal cell populations \citep{NUNEZ2006, SILVA2013, Pesaran2018, Ilmoniemi2019}.
%On the other hand, we know very little about exactly which types of neural activity that cause even the most well-studied characteristics of the EEG signal, such as different types of oscillations (e.g., alpha, beta, and gamma waves) and stereotyped EEG-shapes in response to sensory stimuli (event-related potentials, ERPs) \citep{COHEN2017}. Importantly, these different EEG characteristics are affected in predictable ways by various brain conditions, such as sleep and attention \citep{Klimesch1998, Palva2011, Siegel2012}, and by brain disorders including epilepsy and schizophrenia \citep{Niedermeyer2003, Light2013, Freestone2015, MAKI2019}. 
%In other words, a better insight into how different types of brain activity is reflected in EEG signals could help us in understanding the human brain and curing brain diseases  \citep{Uhlirova2016, COHEN2017, MAKI2019}.
%\sntxt{
%As mentioned above, the link between cortical current dipole moments and EEG signals is well established through forward modeling. Such forward models lay the basis for inverse models, aiming to estimate current dipoles from EEG data. Understanding the link between EEGs and neural activity requires a forward model that also includes the link between neural activity and current dipole moments. Forward models bridging the gap between detailed neural activity and EEG signals, would thus open for improved inverse models.
%}
%
%\snnote{suggestion 3:}
%Electroencephalography (EEG) is one of the most important non-invasive methods for studying human cognitive function and diagnosing brain diseases \citep{COHEN2017, Pesaran2018}.  Yet, we know surprisingly little about the neural origin of these electric scalp potentials \citep{COHEN2017}. \sntxt{ On the one hand, we have a two-step forward modeling scheme consisting of i) modeling current dipole moments from neural activity and ii) modeling EEG signals from current dipole moments. While the second step is well-established forward model constructing the basis for inverse modeling of dipolar sources from EEG data, the first step has not been taken full advantage of. This means that
%\sout{
%: On the one hand, we have a relatively good understanding of the biophysics of EEGs, knowing that these signals mainly originate from large numbers of synaptic inputs to cortical pyramidal cell populations}} \citep{NUNEZ2006, SILVA2013, Pesaran2018, Ilmoniemi2019}.
%\sntxt{\sout{On the other hand,}} we know very little about exactly which types of neural activity that cause even the most well-studied characteristics of the EEG signal, such as different types of oscillations (e.g., alpha, beta, and gamma waves) and stereotyped EEG-shapes in response to sensory stimuli (event-related potentials, ERPs) \citep{COHEN2017}. Importantly, these different EEG characteristics are affected in predictable ways by various brain conditions, such as sleep and attention \citep{Klimesch1998, Palva2011, Siegel2012}, and by brain disorders including epilepsy and schizophrenia \citep{Niedermeyer2003, Light2013, Freestone2015, MAKI2019}. 
%In other words, a better insight into how different types of brain activity is reflected in EEG signals could help us in understanding the human brain and curing brain diseases  \citep{Uhlirova2016, COHEN2017, MAKI2019}.
%
%\gen{I also think the possibility for making better inverse models could be mentioned as a motivation.}

Electroencephalography (EEG) is one of the most important non-invasive methods for studying human cognitive function and diagnosing brain diseases \citep{COHEN2017, Pesaran2018}.  Yet, we know surprisingly little about the neural origin of these electric scalp potentials \citep{COHEN2017}: On the one hand, we have a relatively good understanding of the biophysics of EEGs, knowing that these signals mainly originate from \sntxt{cortical dipoles. This has been taken advantage of for a long time in source localization, by inverse modeling of the underlying cortical current dipoles from EEG data. On the other hand, even though these dipoles are assumed to originate from }
large numbers of synaptic inputs to cortical pyramidal cell populations \citep{NUNEZ2006, SILVA2013, Pesaran2018, Ilmoniemi2019},
\sntxt{the precise biophysical link between cortical dipoles and the underlying neural activity has remained unclear. In other words, w}e know very little about exactly which types of neural activity that cause even the most well-studied characteristics of the EEG signal, such as different types of oscillations (e.g., alpha, beta, and gamma waves) and stereotyped EEG-shapes in response to sensory stimuli (event-related potentials, ERPs) \citep{COHEN2017}. Importantly, these different EEG characteristics are affected in predictable ways by various brain conditions, such as sleep and attention \citep{Klimesch1998, Palva2011, Siegel2012}, and by brain disorders including epilepsy and schizophrenia \citep{Niedermeyer2003, Light2013, Freestone2015, MAKI2019}. 
\sntxt{\sout{In other words, }This means that} a better insight into how different types of brain activity is reflected in EEG signals could help us in understanding the human brain and curing brain diseases  \citep{Uhlirova2016, COHEN2017, MAKI2019}.
\gen{I also think the possibility for making better inverse models could be mentioned as a motivation.}
...
and the EU Human Brain Project \citep{ALLEN, HBP, EINEVOLL2019}. The possibility to calculate EEG signals from such existing and future large-scale biophysically detailed neural simulations could lead to valuable insights into the neural origin of the EEG.
...
Here, we introduce an approach for %\sout{automated} \gen{Synes kanskje ikke "automated" er nodvendig.} 
\sntxt{\sout{reduction of} reducing} arbitrary biophysically detailed simulated neural activity to current dipoles, \sntxt{which represents an enormous reduction of modeling complexity. Further, we verify how the approach is useful\sout{ and verify its applicability}} for EEG signal calculations \sntxt{, bur less so for ECoG predictions. Next, we look into \sout{. Further, we show}} how the approach \sntxt{\sout{is useful} can be applied} for investigating the origin of EEG signals,\sntxt{ particularly for revealing calcium spikes,} before demonstrating how our methods can be applied for pre-existing large-scale network models. Finally, we show how current dipoles can be combined with realistic and complex head models, which enables simulation of EEG signals with unprecedented biophysical detail.  
...
\sntxt{Neural simulations} shown in Fig.~\ref{fig:dipole_field}-\ref{fig:ca_spike} \sntxt{received} synaptic input modeled as conductance-based, \sntxt{two-}exponential synapses (\texttt{Exp2Syn} in \texttt{NEURON}). The rise time constant was set to 1 ms and the decay time constant was 3 ms, synaptic reversal potential was 0 mV and the synaptic weight was set to 0.002~\si{\mu S}, unless otherwise specified. %\tvnnote{Gjelder ikke hybrid modellen. Kanskje gi figurnummer?}
...
We introduce an approach for modeling electroencephalography (EEG) signals from detailed biophysical multicompartment cell models. For illustration, we first consider single synaptic input to single neurons in an infinite homogeneous head model, before moving on to a simple, generic head model. \sntxt{We also look into the possibility for ECoG modeling with this approach.} Finally, we study EEGs from population activity, also applying a realistic head model. Note that while we will only consider EEG signals, the general approach is equally valid for MEG signals. 
...
\begin{figure}[H]
	...
	\caption{\textbf{Extracellular potentials become dipolar in the far field limit}. 
		\textbf{A}: \sntxt{Passive l}ayer 2/3 pyramidal cell from human \citep{EYAL2016} with an excitatory\sntxt{, conductance-based, two-exponential} synapse placed on apical dendrite (red dot), \sntxt{see \ref{subsec:simulation}. The} resulting transmembrane currents for each compartment \sntxt{are shown as} blue \sntxt{arrow (input current)} and red arrows \sntxt{(return currents)\sout{for negative and positive currents respectively}}.
...
(Fig.~\ref{fig:compare_multi_single_dipole}\textbf{B}, full versus dashed lines for two \sntxt{selected} synapse locations). 
We quantified the model dissimilarities by looking at the relative error, and for a chosen distal synaptic input the relative error was 40.0\% and 1.06\% at the position of the ECoG and EEG electrodes respectively (Fig.~\ref{fig:compare_multi_single_dipole}\textbf{C}, green line). For a specific proximal synaptic input we observed a relative error of 76.1$\%$ at the ECoG position, and 7.61$\%$ at the EEG position (Fig.~\ref{fig:compare_multi_single_dipole}\textbf{C}, purple line). Inserting a single strong synaptic current (synaptic weight 0.05~\si{\mu S}) into the soma \sntxt{of the same layer-2/3 pyramidal cell with active mechanisms \citep{EYAL2018}}, resulting in a somatic spike, gave relative errors of 34.7$\%$ and 0.967$\%$ for the computed ECoG and EEG signals, respectively (results not shown).
...
\sntxt{In order to compare the single-dipole to the mutli-dipole approach, we computed the global error, meaning the absolute difference between the results from the two approaches, normalized by the global maximum EEG magnitude computed with the multi-dipole approach.}
The single-dipole approach gave a maximum global 
...
\gen{Var det ikke nylig en modelleringsartikkel med Riera og co paa dette (som Suzuki tipset oss om)? Er den relevant her?}
...
\gen{Synes det boer sies litt klarere ett sted at "jittered"-effekten er en nettverkseffekt.}
...
It might initially seem surprising that the dendritic calcium spike is so strongly reflected in the single-cell current dipole moment, given that the transmembrane currents associated with the somatic action potential are much larger than those associated with the dendritic calcium spike: the maximum amplitude of the transmembrane currents of the somatic compartment was 45.1~$\si{\nA}$, compared to just 0.30~$\si{\nA}$ for the compartment with the apical synaptic input (Fig.~\ref{fig:ca_spike}{\bf A}, blue and orange dots). 
\gen{Blir denne sammenligningen illustrativ naar det er saa stor forskjell paa arealet kompartmentene?} 
\gen{Og er ikke den storste stroemmen i akkurat disse kompartmentene den fra de synaptiske inputstroemmen og ikke en returstroem?}
...
\begin{figure}[H]
	...
	\caption{\textbf{Current dipole moment expose dendritic calcium spikes.}
		...
		\textbf{B4, C4}: Sum of 1000 instances of the single-cell current dipole moment (from \textbf{B3, C3}), that has been randomly shifted in time with a normally distributed shift with a standard deviation of 10~ms. \gen{Igjen burde vel de synaptiske str{\o}mmene bli spesifisert, selv om de ikke er hovedpoenget her?}
	}
	\label{fig:ca_spike}
\end{figure}
...
The construction of such \sntxt{complex} head models is typically dependent on expensive equipment, %\snnote{MRI er vel det essensielle "expensive equipment" her, saa ville ikke tenkt paa MRI som et eksempel, men heller hovedutstyret? Isaafall ville jeg skrevet "that is" isteden for "such as". Eller er det andre dyrt utstyr som ogsaa er viktig?}
...
Note that despite the complexity, the NYH model is substantially faster than the four-sphere model. In order to simulate the EEGs from a dipole moment vector containing 1200 timesteps, the NYH model execution times were \sntxt{$\sim$}~0.4~s, while the four-sphere model needed \sntxt{$\sim$}~1.5~s. %\gen{Bedre aa si $>$1.X s, ogsaa 0.2s er jo mindre enn 1.5s ..:-)}
...
\begin{figure}[H]
	...
	\caption{\textbf{EEG signals from neuron population can be modeled with the four-sphere model and the New York Head}. EEG signals from population dipole resulting from waves of synaptic input to the cortical microcircuit model from \cite{POTJANS2014}. Population dipole was placed in two different locations: parietal lobe (C,D,G,I) and occipital lobe (E,F,H,J).
		...
		{\bf J}: Equivalent to panel I, however, EEG signals from panel E, F generated by dipole in occipital lobe.
...
%As illustrated in \cite{MAKI2019}, modeling EEG signals from simulated neural activity through calculation of the current dipole moment, will be an important brick when bridging the gap between individual genes and clinical EEG signal phenotypes.
