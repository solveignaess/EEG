...
\end{equation}
when the distance R from the center of the volume to the measurement point is larger than the distance from the volume center to the most peripheral source \citep{JACKSON1998}. For definitions of the monopole contribution $C_{\text{monopole}}$, dipole contribution $C_{\text{dipole}}$, quadrupole contribution $C_{\text{quadrupole}}$, see \cite{RIERA2012}.
...
A current dipole moment ${\bf p}$ can be calculated from an axial current $I$ inside a neuron and the distance vector ${\bf d}$ traveled by the axial current: ${\bf p} = I{\bf d}$, analogous to a charge dipole moment.
...
From a multicompartmental neuron simulation, one can compute multiple current dipole moments: e.i. one for each axial current flowing between neighboring compartments in the neuron:
\begin{equation}
...
In order to save time and storage, we can compute a single current dipole moment for each neuron. One way of doing this is to sum up the multiple current dipole moments, in other words, to make a position-weighted sum of all the axial currents for each time point in the neuron simulation:

\begin{equation}\label{eq:dip_sum_axial}
\mathbf{p}(t) = \sum_{k=1}^{N-1} {\bf p}_k(t) = \sum_{i=1}^{N-1} I_k^{axial}(t) \mathbf{d}_k,
\end{equation}
where $N$ is the number of compartments and $N-1$ is the number of axial currents. Combining this single current dipole moment with the current dipole approximation gives us the extracellular potential from a single neuron in the far-field limit ($R>3d$ or $4d$) \citep{NUNEZ2006}.
