...
radial symmetry will tend to make the orthogonal components of the current dipole moment cancel \citep{HAGEN2018}. 
In contrast, interneurons show much less of a preferred orientation
(cite), and are therefore not expected to give any meaningful contribution to the EEG signal (cite) (except indirectly, by giving synaptic input to pyramidal cells). 
...
We used two different cortical pyramidal cell models, that is, the previously used passive human layer 2/3 cell from \cite{EYAL2016}, and in addition a rat layer 5 cell model from \cite{HAY2011}, which has 10 different active conductances and capability for both somatic and dendritic spikes (but note that for now, all synaptic input was kept subthreshold) \tvnnote{Need good way to refer to the specific morphology in Figure 3}. 
...
%For all cases the components of the current dipole moment ($\mathbf{p}$) that was orthogonal ($p_x$ and $p_y$) to the depth axis of cortex ($z$) was negligible compared to the $p_z$-component (Fig.~\ref{fig:eeg_compare_cell_types}\textbf{C}), as expected from  This means that we can describe the single-cell EEG contribution fully in terms of a single time-dependent array, instead of in terms of the same number of current sources/sinks as the number of compartments (typically ~ 1000 for a large pyramidal cell).
