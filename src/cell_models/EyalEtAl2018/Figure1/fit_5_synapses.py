...
########
#
# This code fits the synaptic conductance of five synapses to fit one experimental EPSP
...
secs_ix = []
segs = []
for i in range((len(sys.argv)-2)/3):
    print sys.argv[3+3*i:3+3*(i+1)]
    trees.append(sys.argv[3+3*i])
    secs_ix.append(int(sys.argv[4+3*i]))
...
for i in range(NUMBER_OF_SYNAPSES):
    ...
    Synlist[i].e=E_SYN
    Synlist[i].tau1=TAU_1
    Synlist[i].tau2=TAU_2
    Conlist.append(h.NetCon(Stim1,Synlist[i]))
    Conlist[i].weight[0] = INIT_WEIGHT
    Conlist[i].delay = DELAY
...
try:
    filename_output = PATH_w_res+expname+"/"+str(c)+".txt"
    with open(filename_output,'w') as f:
        f.write(str(NUMBER_OF_SYNAPSES))
        for i in range(NUMBER_OF_SYNAPSES):
            f.write(","+",".join([trees[i],str(secs_ix[i]),str(segs[i])]))
        f.write(","+str(weight)+","+str(spike_time)+","+str(RMSD))
