...
# in order to generate NMDA spike in each of the model terminals
# For each terminal, it activates between 1 and 50 synapses and measure how much time the local voltage was above -40 mV. 
# In Eyal et al, we defined NMDA spikes as events where the local voltage is above -40 for at least 20 ms
#
# it may take half an hour to run
#
...
h("objref cell, tobj")
morph_file = "../../Morphs/2013_03_13_cell03_1204_H42_02.ASC"
model_file = "cell1303_03_model_cm_0_43"
...
output_file_t_thresh = "num_syn_per_seg_results/num_syn_per_seg_time_threshold_terminals_" + model_file[:11] + ".txt"	
...
for sec in list(HCell.basal)+list(HCell.apical):
	...
	for S in range(1,51):
		...
		dendV = h.Vector()
		tV = h.Vector()
		add_spines_on_seg(S,sec,x)
		set_AMPA_NMDA_synapses(S)	
		dendV.record(sec(x)._ref_v)
		tV.record(h._ref_t)
		...
		if S ==50:
			print x,len(time_above_threshold),MAXV


	s_time_thresh += sec.hname()+","+str(x)+","+",".join([str(t) for t in time_above_threshold])+"\n"
