...
ax_NYH = fig.add_axes([xstart + 0.02, 0.73, 0.15, 0.22], aspect=1,
                      title='New York head', frameon=False,
                      xticks=[], yticks=[]) # NYH model
...
ax8 = fig.add_axes([xstart+0.41, ystart_3, 0.15, ywidth],
                   xlabel="t (ms)", ylabel=r'$\Phi$ (µV)') # closest EEG occipital
ax5 = fig.add_axes([xstart+0.66, ystart_2, 0.15, ywidth],
                   ylabel='|$\Phi$($t = t_{max}$)|  (µV)') # EEG at electrodes parietal
ax9 = fig.add_axes([xstart+0.66, ystart_3, 0.15, ywidth],
                   xlabel='electrode distance (mm)',
                   ylabel='|$\Phi$($t = t_{max}$)| (µV)') # EEG at electrodes occipital
...
cbar.set_label(r'$\Phi$ (µV)', labelpad=-1, fontsize=10)
