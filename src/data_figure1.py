...
def make_data(morphology, syninds, dip_loc=None, cell_model=None, x_rot=0, y_rot=0, z_rot=0, active=False):
    ...
    Y = np.zeros(X.shape)
    # simulate cell
    cell_parameters, synapse_parameters, grid_electrode_parameters = set_parameters(morphology, X, Y, Z, sigma=sigma,cell_model=cell_model)
    ...
    cb_LFP_close = grid_electrode.LFP*k
    ...
    multi_dip_LFP_close = inf_vol.get_multi_dipole_potential(cell, gridpoints_close)*k
    ...
    db_LFP_close = inf_vol.get_dipole_potential(single_dip , gridpoints_close - r_mid)*k
    ...
    cb_LFP_far = grid_electrode_far.LFP*k
    ...
    time_max = np.argmax(np.abs(np.linalg.norm(cell.current_dipole_moment, axis=1)))
    LFP_max = 100. #nV #np.round(np.max(np.abs(grid_electrode_LFP[:, time_max])))

    return cell, cb_LFP_close, cb_LFP_far, multi_dip_LFP_close, multi_dip_LFP_far, db_LFP_close, db_LFP_far, LFP_max, time_max, multi_dips, multi_dip_locs, single_dip, r_mid, X, Z, X_f, Z_f
...
if __name__ == '__main__':
    ...
    # generate data
    cell, cb_LFP_close, cb_LFP_far, multi_dip_LFP_close, multi_dip_LFP_far, db_LFP_close, db_LFP_far, LFP_max, time_max, multi_dips, multi_dip_locs, single_dip, r_mid, X, Z, X_far, Z_far = make_data(morphology, syninds, x_rot=xrot, y_rot=yrot)
